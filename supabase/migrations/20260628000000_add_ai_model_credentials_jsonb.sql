-- Migration: Add ai_model_credentials JSONB column to store_settings and ai_settings
-- Date: 2026-06-28
-- Description: Store per-provider API keys as JSON instead of flat content_keys/vision_keys

-- 1. Add JSONB column to store_settings
ALTER TABLE store_settings
ADD COLUMN IF NOT EXISTS ai_model_credentials JSONB DEFAULT '{}'::jsonb;
...
-- Structure: { "content": { "<provider>": "<keys>" }, "vision": { "<provider>": "<keys>" } }
UPDATE store_settings
SET ai_model_credentials = jsonb_build_object(
  'content', jsonb_build_object(content_provider, content_keys),
  'vision', jsonb_build_object(vision_provider, vision_keys)
)
WHERE ai_model_credentials = '{}'::jsonb
  AND (content_keys IS NOT NULL AND content_keys <> ''
    OR vision_keys IS NOT NULL AND vision_keys <> '');
...
-- 4. Update sync_settings_to_ai trigger function to include ai_model_credentials
...
-- 5. Update sync_ai_to_settings trigger function to include ai_model_credentials
